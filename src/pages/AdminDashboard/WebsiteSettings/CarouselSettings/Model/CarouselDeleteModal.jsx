    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fadeIn">
      <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl border border-[#FED7AA] animate-scaleIn">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-xl font-bold text-[#7C2D12]">Delete Carousel Item</h3>
        </div>
        
        {selectedItem && (
          <div className="mb-4 p-4 bg-[#FFEDE9] rounded-lg border border-[#FED7AA]">
            <div className="flex items-center gap-3 mb-3">
              {selectedItem.image_url && (
                <img
                  src={selectedItem.image_url}
                  alt={selectedItem.title}
                  className="w-16 h-16 object-cover rounded-lg border border-[#FED7AA]"
                />
              )}
              <div>
                <h4 className="font-semibold text-[#7C2D12]">{selectedItem.title}</h4>
                <p className="text-sm text-[#9A3412]">Order: {selectedItem.order}</p>
              </div>
            </div>
        ...
        <p className="text-[#9A3412] mb-6">
          Are you sure you want to delete this carousel item? This action cannot be undone and the item will be permanently removed from the carousel.
        </p>
        ...
        <div className="flex gap-3 justify-end">
          ...
          >
            {actionLoading === `delete-${selectedItem?.id}` ? (
              <>
